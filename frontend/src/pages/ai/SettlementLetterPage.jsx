...
import { ArrowLeft, FileText, Copy, Download, RefreshCw, AlertTriangle, Sparkles, Check } from 'lucide-react';
import { useLoans } from '@/hooks/useLoans';
import { aiApi } from '@/api/ai';
import { renderMarkdown } from '@/utils/markdown';
import toast from 'react-hot-toast';

export default function SettlementLetterPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await aiApi.generateSettlementLetter(
      ...
      setResult(payload);
      toast.success('Settlement proposal letter generated!');
    } catch (err) {
  ...
  const handleCopy = () => {
    if (!result?.letter_content) return;
  ...
  const handleDownload = () => {
    if (!result?.letter_content) return;
    const blob = new Blob([result.letter_content], { type: 'text/markdown;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `settlement_letter_loan_${selectedLoanId}.md`);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-surface-900 dark:text-surface-100 flex items-center gap-2">
            <FileText size={20} className="text-teal-500" /> Settlement Letter
          </h1>
          <p className="text-sm text-surface-500 dark:text-surface-400">
            Generate formal request letters specifying settlement compromises.
          </p>
        </div>
      </div>
        ...
        <Card padding="p-5" className="lg:col-span-1 h-fit space-y-4">
          ...
          {loansLoading ? (
            ...
            <div className="text-xs text-amber-500 flex gap-2 items-center">
              <AlertTriangle size={14} /> Add active loans before generating letters.
            </div>
          ) : (
            ...
            {isGenerating ? (
              ...
              <div className="flex flex-col h-full justify-between gap-5">
                <div className="flex justify-between items-center pb-3 border-b border-surface-100 dark:border-surface-700/50 flex-wrap gap-2">
                  <Badge variant="teal">Letter Generated</Badge>
                ...
                <div className="flex-1 overflow-y-auto max-h-[500px] pr-2">
                  {renderMarkdown(result?.letter_content)}
                </div>
              </div>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center py-16 text-surface-400 dark:text-surface-500 text-center gap-2">
                <FileText size={36} className="text-surface-300 dark:text-surface-600" />
