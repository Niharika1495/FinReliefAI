...
import { ArrowLeft, Mail, Copy, Download, RefreshCw, AlertTriangle, Sparkles, Check } from 'lucide-react';
...
export default function NegotiationEmailPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await aiApi.generateNegotiationEmail(
      ...
      setResult(payload);
      toast.success('Negotiation email drafted!');
    } catch (err) {
  ...
  const handleCopy = () => {
    if (!result) return;
    const combined = `Subject: ${result.subject}\n\n${result.email_body}`;
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.setAttribute('download', `negotiation_email_loan_${selectedLoanId}.txt`);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-surface-900 dark:text-surface-100 flex items-center gap-2">
            <Mail size={20} className="text-blue-500" /> Negotiation Email
          </h1>
          <p className="text-sm text-surface-500 dark:text-surface-400">
            Generate creditor correspondence proposal drafts.
          </p>
        </div>
      </div>
        ...
        <Card padding="p-5" className="lg:col-span-1 h-fit space-y-4">
          ...
          {loansLoading ? (
            ...
            <div className="text-xs text-amber-500 flex gap-2 items-center">
              <AlertTriangle size={14} /> Add active loans before drafting emails.
            </div>
          ) : (
          ...
          <Card padding="p-6" className="min-h-[300px] flex flex-col justify-between">
            {isGenerating ? (
              <div className="flex-1 flex flex-col items-center justify-center py-16 gap-3">
                <Spinner size="lg" />
                <p className="text-sm text-surface-500 animate-pulse">Drafting message...</p>
              </div>
            ) : result ? (
              <div className="flex flex-col h-full justify-between gap-5">
                <div className="flex justify-between items-center pb-3 border-b border-surface-100 dark:border-surface-700/50 flex-wrap gap-2">
                  <Badge variant="info">Email Drafted</Badge>
                ...
                <div className="flex-1 space-y-4 overflow-y-auto max-h-[500px] pr-2">
                  <div className="bg-surface-50 dark:bg-surface-850 p-3.5 rounded-xl border border-surface-150 dark:border-surface-750">
                    <span className="text-xs text-surface-400 font-semibold uppercase tracking-wider block mb-0.5">Subject</span>
                    <span className="text-sm font-semibold text-surface-900 dark:text-surface-100">{result.subject}</span>
                  </div>
                  
                  <div className="bg-surface-50 dark:bg-surface-850 p-4 rounded-xl border border-surface-150 dark:border-surface-750">
                    <span className="text-xs text-surface-400 font-semibold uppercase tracking-wider block mb-2">Message Body</span>
                    <div className="text-sm whitespace-pre-wrap leading-relaxed text-surface-700 dark:text-surface-200">
                      {result.email_body}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-16 text-surface-400 dark:text-surface-500 text-center gap-2">
                <Mail size={36} className="text-surface-300 dark:text-surface-600" />
                <p className="text-sm">Enter configuration parameters and click Draft Email.</p>
              </div>
            )}
          </Card>
        </div>
